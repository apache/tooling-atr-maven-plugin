...
@Named
@Singleton
public class AtrClientFactory {

    private final Logger logger = LoggerFactory.getLogger(AtrClientFactory.class);
    ...
    @Inject
    AtrClientFactory(Provider<MavenSession> mavenSessionProvider, SettingsDecrypter settingsDecrypter) {
    ...
    public AtrClient createAtrClient(URL baseUrl, String serverId) throws AtrClientException {
        ...
        AtomicReference<String> jtwCache = (AtomicReference<String>) session.getRepositorySession()
                .getData()
                .computeIfAbsent(dataKey(baseUrl, serverId), AtomicReference::new);
        Server server = getServer(session, serverId);
        return new AtrClient(baseUrl, server.getUsername(), server.getPassword(), jtwCache);
    }
